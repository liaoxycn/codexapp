...
| 归档会话查询收口 | App 端不查询、不展示已归档会话。 | 我打开 App 抽屉或手动刷新；我期望只看到 Desktop 主列表里的非归档会话，归档后的会话立即从移动端目录消失。 | 主动排查发现 gateway hydration 会额外调用 `thread/list archived=true`，并把归档会话合进移动端 snapshot；归档动作本地也可能残留刚归档的会话。 | gateway 已停止查询归档列表，只调用 `threadList(false)`；归档后删除本地缓存并回草稿态；Android snapshot mapper 兜底过滤 `archived=true`；测试已改为断言不查询/不展示归档会话。 |
...
| App 更新检查 | App 启动后异步检查 GitHub Releases，有新版本时在抽屉顶部提示并支持下载/安装 APK。 | 我打开 App，后台自动查询 `liaoxycn/CodexMobileApp` 最新 release；如果 release 版本大于本地版本，我打开抽屉应在 gateway 状态下方看到小号警告色更新提示。点“下载更新”后看到进度条；下载完成点“立即安装”，系统弹出 APK 安装界面；若未授予未知应用安装权限，先跳到系统设置授权。 | App 原来没有自更新能力；移动端用户只能手动找 release 下载。Android 8+ 安装外部 APK 还需要 `REQUEST_INSTALL_PACKAGES` 和系统“安装未知应用”授权，直接 `file://` 也不能给系统安装器读取。 | 新增本地 `AppUpdateManager`，启动时用 GitHub latest release API 查询 `tag_name` 和 `.apk` asset，按语义版本比较；下载 APK 到 App external files；通过 `FileProvider` 生成 `content://` 并授予读权限调用系统安装器；Manifest 增加 `REQUEST_INSTALL_PACKAGES` 和 provider；抽屉提示复用 poke 风格并显示下载进度；单测覆盖版本比较。 |
...
| 会话数对齐 Desktop | 移动端列表数量应和 Codex Desktop 主列表一致，不把历史扫描结果全量展示。 | 我对比 Desktop 右侧会话列表和 App 抽屉；我期望只看到 Desktop 主列表里的项目/普通会话，例如 `md2html/项目会话测试`、`codexapp` 相关会话、`查看深圳天气`，不出现 `OpenManus`、`trtr`、`empty`、`mychat` 等 Desktop 未展示历史项。 | 实测 `thread/list archived=false` 默认返回 18 条，Desktop 截图约 7 条；多出来的不是 `sourceKinds` 问题，`OpenManus/empty/trtr` 也是 `source:"vscode"`。继续查到 `.codex/external_agent_session_imports.json` 含外部导入线程，`.codex-global-state.json` 的 `heartbeat-thread-permissions-by-id` 才接近 Desktop 主列表索引。 | gateway 目录层新增 Desktop 可见性过滤：排除 `cli/exec/unknown/custom`、外部导入线程、`<environment_context>` 占位线程，以及未进入 Desktop heartbeat 索引的 `vscode` 历史线程；保留 `appServer` 手机新建线程。hydration 和 runtime summaries 双层过滤，回归测试覆盖。 |
| 分叉入口迁移到 turn 回复 | 会话分叉不放在会话列表，也不放在用户发送消息上，改为对齐 Desktop：从助手回复消息操作。 | 我打开某个会话，在最终助手回复底部点“从此处分叉”图标；我期望新会话只保留到这轮回复为止，用户消息菜单和会话列表菜单都不出现分叉，且新分叉会话立刻出现在 App 抽屉并显示为空闲。 | 调研 app-server 协议发现 `thread/fork` 只能按 `threadId` 分叉，没有 `turnId`；会话列表入口语义不对；继续对齐 Desktop 后确认入口应在 turn 的助手回复消息。用户实测发现 Desktop 已出现分叉会话但 App 看不到，根因是 gateway 的 Desktop 主列表过滤只认 heartbeat 索引，新 fork 的 `vscode` 会话在索引刷新前被过滤掉；继续实测又发现分叉会话默认“进行中”，根因是 fork 会更新 `updatedAt`，gateway 把“更新时间晚于最后完成 turn”的历史完成会话误判成未物化活动。 | 抽屉会话菜单和用户消息菜单均已移除“分叉”；gateway 仅给助手消息下发 `forkNumTurns`；App 从最终助手回复底部图标发送 `fork_thread.numTurns`；gateway 先 `thread/fork`，必要时对 fork 后线程 `thread/rollback` 裁掉后续 turns；App 主动创建/分叉出的本地目录项会临时绕过 heartbeat 过滤；未物化活动兜底只用于 `notLoaded` 状态，fork 后已完成历史 turn 不再显示进行中；已补 Node/Android 回归测试。 |
...
| 消息流抖动/状态同步 | 发送 hello 后消息流应稳定显示，不被旧刷新覆盖。 | 我在 md2html 项目“项目会话测试”里发送 `hello`；我期望用户消息和助手 turn 稳定出现，不时隐时现，也不因为后台刷新跳回旧会话。 | App 侧每 1.2 秒主动 `refresh_threads` 与 gateway/app-server 推送并行，可能用旧快照覆盖实时消息；gateway 刷新期间若用户手动切会话，旧刷新会晚到并覆盖新选择。 | App 侧 live 自动刷新已停用，只保留手动刷新动画；gateway 在任何 await 前捕获选择版本，刷新返回时如用户已切换则返回当前快照，不再激活旧线程；已补 Android/Gateway 回归测试。 |
...
| 桌面端重启提示 | 移动端改动会话后只提示用户手动重启 Codex Desktop，不再自动 poke/刷新；有运行中会话时不显示提示。 | 我在 App 里重命名、归档、恢复、分叉或发送消息；如果当前没有运行中的会话，我打开抽屉应在 gateway 连接状态下方看到小号警告色“会话有变更，是否立即重启 Codex 桌面端”，点击后 gateway 执行重启并清除提示；如果还有会话运行中，则暂不打扰。 | 旧 `/poke` 会自动聚焦 Desktop 并发送 Esc/Ctrl+R/Ctrl+Shift+R/F5，发送消息后也会自动触发，既不可靠也打断用户；后来又需要避免运行中会话时诱导重启。 | 已保留 `/poke` 接口但改为重启 Codex Desktop 进程；gateway 对移动端会话变更只记录 `desktopRestartRequired` 并下发 snapshot/patch；App 抽屉提示需同时满足“有变更且无 running 会话”；点击发送 `restart_desktop`，成功后清标记。Gateway/Android 单测已覆盖协议、patch、命令发送和提示状态映射。 |
...
| 归档/分叉/重命名 | 目录只保留非归档会话的归档、重命名；分叉从助手回复消息触发。 | 我在抽屉里重命名或归档会话；我在消息流最终助手回复底部点“从此处分叉”；我期望抽屉收起，归档项从列表消失，分叉后进入新会话且只包含目标 turn 之前上下文。 | 触发操作后抽屉不关闭；归档后会跳到别的会话；旧分叉入口放在会话列表，无法表达目标 turn；曾误放到用户消息菜单；本轮发现 gateway 仍会查询归档列表。 | 已修复抽屉自动收起；归档后回草稿态且不再查询/展示归档会话；会话列表和用户消息分叉入口已删除；助手消息底部图标用 `fork_thread.numTurns` + gateway `fork/rollback` 实现；`protocol:selftest` 已覆盖 rename、archive、unarchive，Node 单测覆盖 turn 分叉。 |
...
- `node scripts/dev-run.mjs`：通过；App 更新检查/下载/安装入口改动后已重新编译安装并打开 App，未卸载。
...
| Desktop 同步提示 | 移动端改动后提示是否重启 Desktop，但有运行中会话时不提示。 | 我从手机重命名/归档/分叉/发送消息，抽屉顶部出现小号警告色重启提示；有运行中会话时不出现。 | `/poke` 旧自动刷新逻辑已删除，保留接口；gateway 记录移动端变更并提供 Desktop 重启提示。 |
| App 更新 | 启动后异步检查 GitHub release。 | 我安装低版本包，打开 App，看到更新提示；点击下载显示进度，完成后拉起系统安装器。 | 已接入 GitHub latest release API、APK 下载进度、FileProvider 和 Android 安装 intent；本地 `0.1.8` 对 latest `0.1.7` 不弹。 |
