...
| patch 降级 | patch revision 不匹配时提示刷新，并等 full snapshot 恢复。 | 我在异常网络/乱序情况下收到过期 patch；我期望 App 不应用错误状态，而是提示刷新并等待完整快照。 | 已确认协议要求存在，但恢复链路需要继续验证是否足够顺滑。 | Android reducer 已校验 `baseRevision`；后续重点检查是否需要自动请求 full snapshot，而不只提示刷新。 |
...
2. 重点复测 patch mismatch 后是否应自动请求 full snapshot。
3. 继续按用户视角找弱反馈问题，例如编辑/重发是否还需要 toast 或视觉确认。
4. 盘点无法实现或未使用的兼容导出层、mock 代码，确认无引用后清理。
