...
| 归档/fork/重命名 | 线程目录操作后当前选中和列表更新正确。 | 我重命名当前会话、fork 出新会话、归档旧会话，再恢复归档会话；我期望抽屉收起，并在主视图看到当前标题/状态更新。 | 触发操作后抽屉不关闭，且协议 selftest 缺少 rename/unarchive。 | 已修复抽屉自动收起；`protocol:selftest` 已覆盖 rename、fork、archive、unarchive。 |
