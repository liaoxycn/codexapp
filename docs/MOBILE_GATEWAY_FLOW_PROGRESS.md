...
| 发布脚本断点恢复 | GitHub 网络中断后发包脚本应能一键恢复。 | 我执行 `node scripts/github-release.mjs -Version ... -VersionCode ... -Notes ...` 发包；如果分支已推送、本地 tag 已创建但 tag push 因网络失败，我修复网络后期望原命令可继续补推，不需要手动删 tag 或拆命令。 | 旧脚本看到本地 tag 已存在会直接退出，导致一次网络 reset 后必须人工判断 tag/HEAD/远端状态。已改为先查远端 tag；远端不存在且本地 tag 指向 HEAD 时进入“push only”恢复路径，继续推分支和 tag；若本地 tag 指向其他提交或远端 tag 已存在仍报错保护。 |
