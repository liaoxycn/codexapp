...
let phase = 0;
let sent = false;
ws.on("open", () => ws.send(JSON.stringify({ type: "hello", client: "approve-probe", version: "0.1.0" })));
ws.on("message", (buf) => {
  ...
  if (phase === 2 && msg.pendingApproval) {
    phase = 3;
    ws.send(JSON.stringify({ type: "approve_pending" }));
    return;
  }
  const hasApproved = tail.some((m) => JSON.stringify(m).includes("审批已允许"));
  const hasCommand = tail.some((m) => JSON.stringify(m).includes("命令:"));
  if (phase === 3 && hasApproved && hasCommand) {
    console.log("APPROVE_FLOW_OK");
    ws.close();
  }
});
