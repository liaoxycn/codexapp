...
let phase = 0;
let sent = false;
ws.on("open", () => ws.send(JSON.stringify({ type: "hello", client: "bang-probe", version: "0.1.0" })));
ws.on("message", (buf) => {
  ...
  if (phase >= 2 && tail.some((m) => JSON.stringify(m).includes("命令:"))) {
    console.log("COMMAND_EXECUTED_WITHOUT_APPROVAL");
    ws.close();
  }
});
