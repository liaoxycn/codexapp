...
- 阶段性发包统一执行 `node scripts/github-release.mjs -Version <x.y.z> -VersionCode <正整数> -Notes "<更新说明>"`，或用 `-NotesFile <路径>` 传更新说明；可选 `-CommitMessage`、`-Branch`、`-Remote`、`-RunChecks`、`-PushRetries <次数>`。脚本会提交当前改动、推送分支和 `v<Version>` tag，tag push 触发 GitHub Actions 打包；脚本执行成功即视为已触发发布，不等待 Actions 结果。
