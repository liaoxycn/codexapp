...
- 已知缺口：
  - `/compact` 真链路已收掉 `思考中` 噪声；但历史线程里已有旧 `上下文已压缩` 项，仍需做一次历史去重清洗。
  - 某些历史线程本身已包含旧探针数据，影响自测可读性；需要增加独立测试线程或清洁线程策略。
  - `turn/completed` 后虽然会回填 `thread/read`，但最终内容同步仍需继续把 `item/completed` / delta / refresh 收敛成一致模型。
  - 高频通知下当前仍是整包 `snapshot` 推送，后续应改成节流快照或增量 patch。
  - Android 真点击“拒绝”还需补一轮稳定自动化；问题不在协议，而在模拟器输入 `! dir` 时 adb 文本注入不稳定。
  - `/goal` 仍未接真实 RPC；本机生成 schema 未暴露对应 client request 类型，需跟随 app-server 版本再核对。
- 下一开发阶段应切到：
  - compact 历史去重与测试线程清洁
  - `item/agentMessage/delta` 增量拼装
  - `turn/completed` 收敛
